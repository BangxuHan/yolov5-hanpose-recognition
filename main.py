...
def handpose_x_process(info_dict, config, is_videos, test_path):
    ...
    # yolo 手部检测模型初始化
    hand_detect_model = yolov5_hand_model(conf_thres=float(config["detect_conf_thres"]),
    ...
    if is_videos:
        ...
        while True:
            ...
            if ret:  # 读取相机图像成功
                ...
                # hand_bbox = hand_detect_model(img, augment=False)[0]
        ...
        for img_name in os.listdir(test_path):
            ...
            algo_img = img.copy()
            hand_bbox = hand_detect_model.predict(img, vis=True)
            # hand_bbox = hand_detect_model(img, augment=False)[0]  # 检测手，获取手的边界框
