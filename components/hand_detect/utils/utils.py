...
def coco80_to_coco91_class():  # converts 80-index (val2014) to 91-index (paper)
    ...
    # a = np.loadtxt('hand/coco.names', dtype='str', delimiter='\n')
    # b = np.loadtxt('hand/coco_paper.names', dtype='str', delimiter='\n')
...
def kmean_anchors(path='./hand/coco128.txt', n=9, img_size=(640, 640), thr=0.20, gen=1000):
    # Creates kmeans anchors for use in *.cfg files: from utils.utils import *; _ = kmean_anchors()
    # n: number of anchors
    # img_size: (min, max) image size used for multi-scale training (can be same values)
    # thr: IoU threshold hyperparameter used for training (0.0 - 1.0)
    # gen: generations to evolve anchors using genetic algorithm
    from components.hand_detect.utils.datasets import LoadImagesAndLabels
