...
def detect(save_img=False):
    ...
    while True:
        ...
        if ret:
            ...
            # cv2.putText(im0, "Yolov5-Detect Hands", (5, im0.shape[0] - 3), cv2.FONT_HERSHEY_DUPLEX, 1.2, (55, 0, 220),
            #             7)
            # cv2.putText(im0, "Yolov5-Detect Hands", (5, im0.shape[0] - 3), cv2.FONT_HERSHEY_DUPLEX, 1.2, (255, 50, 50),
            #             2)
                ...
                if vid_writer is None:  # 记录 demo 视频
                    ...
                    vid_writer = cv2.VideoWriter(save_path + "{}_hand.mp4".format(source.split('.')[0], time_str),
                                                 cv2.VideoWriter_fourcc(*opt.fourcc), fps, (w, h))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='../../weights/yolo_weights/hand_s.pt', help='model.pt path')
    parser.add_argument('--source', type=str, default="../../video158.mp4", help='source')  # file/folder, 0 for webcam
    ...
    parser.add_argument('--save-img', default='runs/detect', action='store_true', help='save results')
